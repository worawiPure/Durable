$(function() {
    ...
    $('#btnSearch_items').on('click', function(e){
        ...
        if(!data.items) {
            ...
            $.ajax({
                type: "POST",
                url: "/general_add_asset/durable_search_items_distribute",
    ...
    $('#btnSearch_type').on('click', function(e){
        ...
        if(!data.type) {
            ...
            $.ajax({
                type: "POST",
                url: "/general_add_asset/durable_search_type_distribute",
                contentType: 'application/json',
                data: JSON.stringify(data)
            })
                .success(function(data) {
    ...
    $('#btnSearch_room').on('click', function(e){
        ...
        data.room = room;
        if(!data.room) {
            alert('กรุณาเลือกห้องด้วยครับ!!')
        } else {
            console.log(data);
            NProgress.start();
            $.ajax({
                type: "POST",
                url: "/general_add_asset/durable_search_room_distribute",
                contentType: 'application/json',
                data: JSON.stringify(data)
            })
                .success(function(data) {
    ...
    $('#btnPrint_items').on('click', function(e){
        ...
        window.open('/prints/report_general_items_distribute/'+items_print)
    });

    $('#btnPrint_type').on('click', function(e){
        e.preventDefault();
        var type_print = $('#slType').val();
        window.open('/prints/report_general_type_distribute/'+type_print)
    });

    $('#btnPrint_room').on('click', function(e){
        e.preventDefault();
        var room_print = $('#slRoom').val();
        window.open('/prints/report_general_room_distribute/'+room_print)
    });
    ...
    $('#btnExport_items_excel').on('click', function(e){
        ...
        window.open('/prints/export_general_items_distribute/'+items_export)
    });

    $('#btnExport_type_excel').on('click', function(e){
        e.preventDefault();
        var type_export = $('#slType').val();
        window.open('/prints/export_general_type_distribute/'+type_export)
    });

    $('#btnExport_room_excel').on('click', function(e){
        e.preventDefault();
        var room_export = $('#slRoom').val();
        window.open('/prints/export_general_room_distribute/'+room_export)
    });
