...
router.post('/save_durable_general', function(req,res){
    ...
    data.receive_date2 = moment(data.receive_date, 'DD/MM/YYYY').format('YYYY-MM-DD');
...
router.get('/report_general_worn-out', function(req,res){
    var db = req.db;
    var data = {};
    if (req.session.level_user_id != 2){
        res.render('./page/access_denied')
        ...
        room.getListRoom(db)
            ...
            .then(function (rows) {
                data.status = rows;
                res.render('./page/general_worn-out', {data: data});
            }, function (err) {
                console.log(err);
                res.render('./page/general_worn-out', {
                    data: { types:[] ,rooms:[],items:[],companys:[],monneys:[],status:[]}
                });
            });
    }
...
router.post('/durable_search_items_worn-out',function(req,res){
    ...
    data.items = req.body.items;
    console.log(data);
    show_detail.search_durable_items_worn_out(db,data)
        .then(function(rows){
            console.log(rows);
            res.send({ok: true,rows:rows});
        },
...
router.post('/durable_search_type_worn-out',function(req,res){
    ...
    data.type = req.body.type;
    console.log(data);
    show_detail.search_durable_type_worn_out(db,data)
        .then(function(rows){
            console.log(rows);
            res.send({ok: true,rows:rows});
        },
...
router.post('/durable_search_room_worn-out',function(req,res){
    ...
    data.room = req.body.room;
    console.log(data);
    show_detail.search_durable_room_worn_out(db,data)
        .then(function(rows){
            console.log(rows);
            res.send({ok: true,rows:rows});
        },
...
router.post('/edit_general_durable', function(req,res){
   ...
   // data.receive_date2=moment(data.receive_date, 'DD/MM/YYYY').format('YYYY-MM-DD');
    data.distribute_date2=moment(data.distribute_date, 'DD/MM/YYYY').format('YYYY-MM-DD');
    console.log(data.distribute_date2);
    if (data.id) {
