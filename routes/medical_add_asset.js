...
var brand = require('../models/durable_medical_manage_systems');
var type = require('../models/durable_medical_manage_systems');
var department = require('../models/durable_medical_manage_systems');
var category = require('../models/durable_medical_manage_systems');
var items = require('../models/durable_medical_manage_systems');
var status = require('../models/durable_medical_manage_systems');
var guarantee =require('../models/durable_medical_manage_systems');
var room =require('../models/durable_medical_manage_systems');
var money =require('../models/durable_medical_manage_systems');
var year =require('../models/durable_medical_manage_systems');
var save = require('../models/durable_medical_manage');
var show_detail = require('../models/durable_medical_manage');
var utils = require('../models/utils');

/* GET home page. */
router.get('/medical_add_asset', function(req,res){
    var db = req.db;
    var data = {};
    if (req.session.level_user_id != 2){
        ...
        type.getListType(db)
            ...
            .then(function (rows) {
                console.log(rows);
                data.status = rows;
                res.render('./page/medical_add_asset', {data: data});
            }, function (err) {
                console.log(err);
                res.render('./page/medical_add_asset', {
                    data: {
...
router.post('/save_durable_medical', function(req,res){
    var db = req.db;
        ...
        save.check_duplicated_items(db,data)
            .then(function (total){
                if (total){
                    ...
                    save.save_asset_medical(db,data)
                        .then(function(rows){
                            console.log('Saved');
                            res.send({ok:true})
                        }, function(err){
...
router.get('/report_medical_distribute', function(req,res){
    var db = req.db;
    var data = {};
    if (req.session.level_user_id != 2){
        res.render('./page/access_denied')
...
router.post('/durable_search_items_distribute',function(req,res){
    ...
    data.items = req.body.items;
    console.log(data);
    show_detail.search_durable_distribute_items(db,data)
        .then(function(rows){
            console.log(rows);
            res.send({ok: true,rows:rows});
        },
...
router.post('/durable_search_type_distribute',function(req,res){
    ...
    data.type = req.body.type;
    console.log(data);
    show_detail.search_durable_distribute_type(db,data)
        .then(function(rows){
            console.log(rows);
            res.send({ok: true,rows:rows});
        },
...
router.get('/durable_medical_show_edit/:id', function(req,res){
    if (req.session.level_user_id != 2){
        res.render('./page/access_denied')
    } else {
        var db = req.db;
        ...
        var data = {};
            show_detail.show_edit_medical_durable(db,id)
                .then(function (rows) {
                    console.log(rows);
                    data.detail = rows[0];
                ...
                .then(function (rows) {
                    data.status = rows;
                    res.render('./page/medical_edit_durable', {data: data});
                }, function (err) {
                    res.render('./page/medical_edit_durable', {
                        data: {
...
router.post('/remove_medical_items', function(req,res){
    var db = req.db;
    var id = req.body.id;
    if(id){
...
router.get('/report_medical_depreciate', function(req,res){
    var db = req.db;
    var data = {};
    if (req.session.level_user_id != 2){
        res.render('./page/access_denied')
        ...
        room.getListRoom(db)
            ...
            .then(function (rows) {
                data.types = rows;
                res.render('./page/medical_search_depreciate', {data: data});
            }, function (err) {
                console.log(err);
                res.render('./page/medical_search_depreciate', {
                    data: { types:[] ,rooms:[]}
                });
            });
    }
